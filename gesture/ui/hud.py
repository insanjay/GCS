...
class CinematicHUD:
    ...
    def draw(self, img, hand_landmarks, state, confidence):
        ...
        for i in range(1, len(self.trail)):
            ...
            cv2.line(
                img, self.trail[i - 1], self.trail[i], config.COLOR_IDLE, thickness
            )
        ...
        if state == "MOVE":
            ...
            radius = 35
            cv2.putText(
                img, "R", (cx - 10, cy + 10), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2
            )
            ...
            radius = 30
            cv2.putText(
                img, "TOGGLE", (cx - 25, cy - 40), cv2.FONT_HERSHEY_PLAIN, 1, color, 1
            )
        ...
        # Main Arc
        cv2.ellipse(
            img, (cx, cy), (radius, radius), self.pulse_phase * 50, 0, 270, color, 2
        )
