    ...
    def run(self):
        ...
        while self.running:
            ...
            if fps < config.FAILSAFE_FPS and (current_time - self.start_time) > 2.0:
                cv2.putText(
                    img,
                    "SAFETY PAUSE: LOW FPS",
                    (config.WINDOW_WIDTH // 2 - 150, config.WINDOW_HEIGHT // 2),
            ...
            if detection_result.hand_landmarks:
                ...
                # Globals
                if not hasattr(self, "drag_active"):
                ...
                if state == "FIST":
                    if (
                        current_time_loop - self.last_toggle_time
                    ) > config.DRAG_TOGGLE_COOLDOWN:
                ...
                # B. NORMAL MODE (Not Dragging)
                    ...
                    if state == "MOVE":
                        ...
                        if (
                            current_time_loop - self.last_click_time
                            ...
                            cv2.circle(
                                img,
                                (
                                    int(norm_x * config.WINDOW_WIDTH),
                                    int(norm_y * config.WINDOW_HEIGHT),
                                ),
                                50,
                                config.COLOR_CLICK,
                                4,
