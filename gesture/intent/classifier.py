...
class GestureClassifier:
    ...
    def process(self, landmarks):
        ...
        thumb_out = (
            math.hypot(thumb_tip.x - index_mcp.x, thumb_tip.y - index_mcp.y) > 0.05
        )
        ...
        dist_index = math.hypot(
            landmarks[4].x - landmarks[8].x, landmarks[4].y - landmarks[8].y
        )
        is_pinch_index = dist_index < config.PINCH_THRESHOLD_NORM

        # Middle Pinch
        dist_middle = math.hypot(
            landmarks[4].x - landmarks[12].x, landmarks[4].y - landmarks[12].y
        )
        ...
        return self.state, {"confidence": 1.0, "pinch_dist": dist_index}
